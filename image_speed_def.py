...
def calculate_features(image_1_cv, image_2_cv, nfeatures):
...
def calculate_mean_distance(list_kp1, list_kp2, matches):
    average_distance = 0
    number_matches = len(matches)

    for c in range(number_matches):
        distance = math.hypot(
            list_kp1[c][0] - list_kp2[c][0], list_kp1[c][1] - list_kp2[c][1]
        )
...
def display_matches(image_1_cv, key_points_1, image_2_cv, key_points_2, matches):
    match_img = cv2.drawMatches(
        image_1_cv, key_points_1, image_2_cv, key_points_2, matches[:100], None
    )
...
def calculate_speed(image_1, image_2, scaling, nfeatures):
    time_difference = get_time_difference(image_1, image_2)
    image_1_cv, image_2_cv = convert_to_cv(image_1, image_2)
    key_points_1, key_points_2, descriptors_1, descriptors_2 = calculate_features(
        image_1_cv, image_2_cv, nfeatures
    )
    matches = calculate_matches(descriptors_1, descriptors_2)
    display_matches(image_1_cv, key_points_1, image_2_cv, key_points_2, matches)
    list_key_points_1, list_key_points_2 = find_matching_coordinates(
        key_points_1, key_points_2, matches
    )
    average_feature_distance = calculate_mean_distance(
        list_key_points_1, list_key_points_2, matches
    )
    speed = calculate_speed_in_kmps(average_feature_distance, scaling, time_difference)
    return speed
